...
class WebScrapingAgent {
  ...
  async findNamespaceByServiceName(serviceName, dynamodbClient) {
    try {
      ...
      // Scan all namespaces to find one that matches the service name
      const scanResult = await dynamodbClient.send(new ScanCommand({
        TableName: 'brmh-namespace',
        FilterExpression: '#data.#nsname = :serviceName OR #data.#nsname = :serviceNameLower OR #data.#nsname = :serviceNameUpper',
        ...
        ExpressionAttributeValues: {
          ...
          ':serviceNameUpper': serviceName.charAt(0).toUpperCase() + serviceName.slice(1).toLowerCase()
        }
      }));

      if (scanResult.Items && scanResult.Items.length > 0) {
        const existingNamespace = scanResult.Items[0];
        console.log(`[Web Scraping Agent] Found existing namespace: ${existingNamespace.data['namespace-id']} for service: ${serviceName}`);
        return existingNamespace.data;
      }
  ...
  async scrapeService(serviceName, options = {}, docClient = null) {
