...
function isExplicitSchemaGenerationIntent(message) {
  ...
  return (
    lower.includes('generate a schema') ||
    lower.includes('create a schema') ||
    lower.includes('edit this schema') ||
    lower.match(/generate.*json schema/) ||
    lower.match(/create.*json schema/)
  );
}
...
const generateSchemaTool = new DynamicStructuredTool({
  ...
  func: async ({ description }) => {
    const prompt = `Generate a JSON schema for the following requirements. Only output the JSON schema, nothing else.\n\n${description}`;
    const outputParser = new JsonOutputParser();
    const chain = RunnableSequence.from([chat, outputParser]);
    const schema = await chain.invoke(prompt);
