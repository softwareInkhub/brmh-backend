...
function isResourceQuery(message) {
  ...
  const questionPatterns = [
    ...
    'which namespace', 'what namespace', 'list namespace', 'show namespace'
  ];
  ...
  const resourceKeywords = [
    ...
    'webhook', 'webhooks', 'account', 'accounts', 'namespace', 'namespaces',
    'pin', 'pins', 'create', 'user', 'order'
  ];
  ...
  console.log('[LLM Agent] isResourceQuery debug for message:', message);
  console.log('[LLM Agent] hasQuestionPattern:', hasQuestionPattern);
  console.log('[LLM Agent] hasResourceKeyword:', hasResourceKeyword);
  console.log('[LLM Agent] isGeneralQuestion:', isGeneralQuestion);
  
  const result = hasQuestionPattern || (hasResourceKeyword && isGeneralQuestion);
  console.log('[LLM Agent] isResourceQuery result:', result);
  
  // Return true if it has a question pattern OR (has resource keyword AND is a question)
  return result;
}

// Function to extract mentioned namespace from message
function extractMentionedNamespace(message) {
  const lowerMessage = message.toLowerCase();
  
  // Common namespace names to look for
  const namespaceNames = [
    'shopify', 'fintech', 'pinterest', 'gmail', 'google', 'youtube', 'github', 
    'razorpay', 'easebuzz', 'whapi', 'aws', 'brmh', 'inkhub', 'todo-app',
    'google sheet', 'google cloud search', 'google docs', 'google calender',
    'google meet', 'gdrive', 'fship', 'influencer marketing', 'brmh project management'
  ];
  
  for (const nsName of namespaceNames) {
    if (lowerMessage.includes(nsName.toLowerCase())) {
      // Map display names to actual namespace IDs
      const namespaceMap = {
        'shopify': 'b429f105-4b19-4ce1-97dd-984e98c72f3c',
        'fintech': 'f04c3ae0-e1ca-4a9b-b017-e121fedbf29b',
        'pinterest': '9c142145-b2eb-4543-ae07-5bfc37bc5fb7',
        'gmail': '100ba755-a003-414e-8e25-fce242255c3d',
        'google calender': '3ddcca7b-4ca9-49f5-86aa-b69ce9edabd1',
        'google docs api': 'bfc92440-8f19-4457-a8f7-050055c2bb52',
        'google sheet': '617b88d4-0ae2-4659-847f-4fee03afacfb',
        'google cloud search': 'f2c0c7a5-99fe-4584-b30a-d284c6e98405',
        'google meet': '9ce2d955-dc4a-4b1f-903e-767b846bfb8c',
        'gdrive': '78ca9477-d228-45bd-b3b9-9ab5cd6c1be6',
        'youtube': '31a7d799-14fe-4fa8-a98c-da1e9c61ad0b',
        'github': 'e71f7c50-5972-4449-8673-6ea6eb1e8e14',
        'razorpay': 'f37a0b05-ebba-425e-9458-93d2c52b96c7',
        'easebuzz api': 'dc978bc5-cf45-4cdd-92a3-795cdd895084',
        'whapi': '20ffd1f8-6ba9-4612-94dc-b2ad771b1741',
        'aws': '32bb2c6f-4978-4eb3-b4fa-ef8901427f3f',
        'brmh': '84a953d2-1f9c-4053-9e3d-040f00ab5e56',
        'inkhub': '2f967e4f-00c7-4c87-a126-c708aa5b0267',
        'todo-app': 'd486dc28-ec2a-4bb8-a07e-ea01ad825b88',
        'fship': '02c89a35-1e93-47c0-b672-ecc343827d8b',
        'influencer marketing': 'c3490482-fcea-478f-804b-32a33f200ce3',
        'brmh project management': '779f7250-b99e-46ca-9462-2e1008a365b8'
      };
      
      return namespaceMap[nsName.toLowerCase()] || null;
    }
  }
...
// Function to handle resource queries with multiple namespaces
async function handleResourceQueryWithMultipleNamespaces(message, allNamespaceData) {
  console.log('[LLM Agent] Handling resource query with multiple namespaces:', message);
  console.log('[LLM Agent] allNamespaceData length:', allNamespaceData.length);
  ...
  // Clean up the resource name - remove "methods", "schemas", etc. to get the namespace name
  if (resourceName) {
    resourceName = resourceName.replace(/\s+(methods?|schemas?|apis?|endpoints?|webhooks?|accounts?)$/i, '').trim();
    // Remove "the" prefix
    resourceName = resourceName.replace(/^the\s+/i, '').trim();
  }
  
  console.log('[LLM Agent] Resource name extracted:', resourceName);
  
  // Determine resource type based on keywords
  if (lowerMessage.includes('schema') || lowerMessage.includes('data model') || lowerMessage.includes('structure')) {
    resourceType = 'schema';
  ...
  } else if (lowerMessage.includes('namespace')) {
    resourceType = 'namespace';
  }
  
  console.log('[LLM Agent] Resource type determined:', resourceType);
  
  // Build comprehensive response for all namespaces
  let response = '';
  
  
  if (resourceType === 'namespace' || lowerMessage.includes('which namespace') || lowerMessage.includes('what namespace')) {
    response = `Here's the context for all namespaces in scope:\n\n`;
    
    allNamespaceData.forEach((nsData, index) => {
      const ns = nsData.namespace;
      const data = nsData.data;
      response += `**${index + 1}. ${ns['namespace-name'] || ns.name || 'Unknown Namespace'}**\n`;
      response += `- **ID**: ${ns['namespace-id'] || ns.id}\n`;
      response += `- **Total Schemas**: ${data.schemas?.length || 0}\n`;
      response += `- **Total Methods**: ${data.methods?.length || 0}\n`;
      response += `- **Total Webhooks**: ${data.webhooks?.length || 0}\n`;
      response += `- **Total Accounts**: ${data.accounts?.length || 0}\n`;
      
      if (data.schemas && data.schemas.length > 0) {
        response += `- **Schemas**:\n`;
        data.schemas.slice(0, 5).forEach((schema) => {
          response += `  • ${schema.schemaName || schema.name || 'Unknown Schema'}\n`;
        });
        if (data.schemas.length > 5) {
          response += `  • ... and ${data.schemas.length - 5} more schemas\n`;
        }
      }
      
      if (data.methods && data.methods.length > 0) {
        response += `- **Methods**:\n`;
        data.methods.slice(0, 5).forEach((method) => {
          response += `  • ${method['namespace-method-name'] || method.methodName || 'Unknown Method'}\n`;
        });
        if (data.methods.length > 5) {
          response += `  • ... and ${data.methods.length - 5} more methods\n`;
        }
      }
      
      if (data.webhooks && data.webhooks.length > 0) {
        response += `- **Webhooks**:\n`;
        data.webhooks.slice(0, 3).forEach((webhook) => {
          response += `  • ${webhook.webhookName || webhook.name || 'Unknown Webhook'}\n`;
        });
        if (data.webhooks.length > 3) {
          response += `  • ... and ${data.webhooks.length - 3} more webhooks\n`;
        }
      }
      ...
      if (data.accounts && data.accounts.length > 0) {
        ...
        data.accounts.slice(0, 3).forEach((account) => {
          response += `  • ${account.accountName || account.name || 'Unknown Account'}\n`;
        });
        if (data.accounts.length > 3) {
          response += `  • ... and ${data.accounts.length - 3} more accounts\n`;
        }
      }
    ...
    response += `You can ask me to generate lambda functions, create documents, or perform other operations using these namespaces and their resources.`;
    
  } else {
    // Handle specific resource queries across all namespaces
    response = `Here's what I found across all namespaces in context:\n\n`;
    
    allNamespaceData.forEach((nsData, index) => {
      const ns = nsData.namespace;
      const data = nsData.data;
      response += `**${ns['namespace-name'] || ns.name || 'Unknown Namespace'}**:\n`;
      
      if (resourceType === 'schema' && data.schemas) {
        // If asking about schemas in a specific namespace, show all schemas
        // If asking about specific schema names, filter by name
        const matchingSchemas = data.schemas.filter((schema) => {
          if (!resourceName) return true; // Show all if no specific name requested
          
          // Check if resourceName matches the namespace name (show all schemas)
          const nsName = ns['namespace-name'] || ns.name || '';
          if (resourceName.toLowerCase() === nsName.toLowerCase()) {
            return true; // Show all schemas for this namespace
          }
          
          // Otherwise filter by schema name
          return (schema.schemaName || schema.name || '').toLowerCase().includes(resourceName.toLowerCase());
        });
        
        if (matchingSchemas.length > 0) {
          matchingSchemas.forEach((schema) => {
            response += `• **Schema**: ${schema.schemaName || schema.name}\n`;
          });
        } else {
          response += `• No matching schemas found\n`;
        }
      } else if (resourceType === 'method' && data.methods) {
        // If asking about methods in a specific namespace, show all methods
        // If asking about specific method names, filter by name
        console.log('[LLM Agent] Processing methods for namespace:', ns['namespace-name']);
        console.log('[LLM Agent] Total methods available:', data.methods?.length || 0);
        console.log('[LLM Agent] Resource name for filtering:', resourceName);
        
        const matchingMethods = data.methods.filter((method) => {
          if (!resourceName) {
            console.log('[LLM Agent] No resource name, showing all methods');
            return true; // Show all if no specific name requested
          }
          
          // Check if resourceName matches the namespace name (show all methods)
          const nsName = ns['namespace-name'] || ns.name || '';
          console.log('[LLM Agent] Comparing resourceName:', resourceName, 'with namespace name:', nsName);
          
          if (resourceName.toLowerCase() === nsName.toLowerCase()) {
            console.log('[LLM Agent] Resource name matches namespace name, showing all methods');
            return true; // Show all methods for this namespace
          }
          
          // Otherwise filter by method name
          const methodName = method['namespace-method-name'] || method.methodName || '';
          const matches = methodName.toLowerCase().includes(resourceName.toLowerCase());
          console.log('[LLM Agent] Method:', methodName, 'matches filter:', matches);
          return matches;
        });
        
        console.log('[LLM Agent] Matching methods found:', matchingMethods.length);
        
        if (matchingMethods.length > 0) {
          matchingMethods.forEach((method) => {
            response += `• **Method**: ${method['namespace-method-name'] || method.methodName} (${method['namespace-method-type'] || method.methodType || 'UNKNOWN'})\n`;
          });
        } else {
          response += `• No matching methods found\n`;
        }
      } else if (resourceType === 'webhook' && data.webhooks) {
        const matchingWebhooks = data.webhooks.filter((webhook) => 
          !resourceName || (webhook.webhookName || webhook.name || '').toLowerCase().includes(resourceName.toLowerCase())
        );
        if (matchingWebhooks.length > 0) {
          matchingWebhooks.forEach((webhook) => {
            response += `• **Webhook**: ${webhook.webhookName || webhook.name}\n`;
          });
        } else {
          response += `• No matching webhooks found\n`;
        }
      } else if (resourceType === 'account' && data.accounts) {
        const matchingAccounts = data.accounts.filter((account) => 
          !resourceName || (account.accountName || account.name || '').toLowerCase().includes(resourceName.toLowerCase())
        );
        if (matchingAccounts.length > 0) {
          matchingAccounts.forEach((account) => {
            response += `• **Account**: ${account.accountName || account.name}\n`;
          });
        } else {
          response += `• No matching accounts found\n`;
        }
      } else {
        response += `• Total resources: ${(data.schemas?.length || 0) + (data.methods?.length || 0) + (data.webhooks?.length || 0) + (data.accounts?.length || 0)}\n`;
      }
...
export async function handleLambdaCodegen({ message, selectedSchema, functionName, runtime, handler, memory, timeout, environment, namespace, allNamespaces = [], res = null, uploadedSchemas = [] }) {
  console.log('[LLM Agent] Enhanced Lambda codegen request:', { message, selectedSchema, functionName, runtime, handler, memory, timeout, environment, namespace, allNamespacesCount: allNamespaces.length, uploadedSchemasCount: uploadedSchemas.length });
  ...
  // If no uploaded schemas or we need more context, check workspace schemas from all namespaces
  if (schemasToUse.length === 0 && !selectedSchema) {
    try {
      console.log('[LLM Agent] No uploaded schemas, searching for relevant schemas in all namespaces...');
      
      // Get schemas from all namespaces in context
      let allAvailableSchemas = [];
      let allNamespaceContexts = [];
      
      if (allNamespaces && allNamespaces.length > 0) {
        console.log('[LLM Agent] Processing schemas from', allNamespaces.length, 'namespaces in context');
        
        for (const ns of allNamespaces) {
          const nsId = ns['namespace-id'] || ns.id;
          if (nsId) {
            try {
              const { schemas: nsSchemas, methods: nsMethods, namespaceInfo } = await getRealNamespaceData(nsId);
              const schemasWithNamespace = nsSchemas.map(schema => ({
                ...schema,
                namespaceName: ns['namespace-name'] || ns.name,
                namespaceId: nsId
              }));
              allAvailableSchemas.push(...schemasWithNamespace);
              allNamespaceContexts.push({
                namespace: ns,
                schemas: nsSchemas,
                methods: nsMethods,
                info: namespaceInfo
              ...
              console.warn('[LLM Agent] Failed to fetch data for namespace', nsId, err.message);
            }
          }
        }
      } else if (namespaceId) {
        // Fallback to single namespace if no allNamespaces provided
        console.log('[LLM Agent] No allNamespaces provided, using single namespace:', namespaceId);
        const { schemas: availableSchemas, methods: availableMethods, namespaceInfo } = await getRealNamespaceData(namespaceId);
        allAvailableSchemas = availableSchemas;
        allNamespaceContexts = [{
      ...
      // Intelligently select relevant schemas based on the message from all available schemas
      const relevantSchemas = await selectRelevantSchemas(message, namespaceId, allAvailableSchemas);
      ...
      if (relevantSchemas.length > 0) {
        ...
        // Build comprehensive namespace context for the prompt
        if (allNamespaceContexts.length > 1) {
          namespaceContext = `**Multiple Namespaces in Context (${allNamespaceContexts.length}):**\n\n`;
          allNamespaceContexts.forEach((ctx, index) => {
            const ns = ctx.namespace;
            const nsName = ns['namespace-name'] || ns.name || 'Unknown';
            const nsId = ns['namespace-id'] || ns.id;
            namespaceContext += `${index + 1}. **${nsName}** (ID: ${nsId})\n`;
            namespaceContext += `   - Schemas: ${ctx.schemas.length}\n`;
            namespaceContext += `   - APIs: ${ctx.methods.map(m => m['namespace-method-name'] || m.methodName).join(', ') || 'None'}\n\n`;
          });
        } else if (allNamespaceContexts.length === 1) {
          const ctx = allNamespaceContexts[0];
          const ns = ctx.namespace;
          namespaceContext = `Current namespace: ${ns['namespace-name'] || ns.name || 'Unknown'}\nNamespace ID: ${ns['namespace-id'] || ns.id}`;
          namespaceContext += `\nAvailable APIs: ${ctx.methods.map(m => m['namespace-method-name'] || m.methodName).join(', ') || 'None'}`;
        } else {
          namespaceContext = `Current namespace: Unknown\nNamespace ID: ${namespaceId}`;
        }
      } else {
...
export const agentSystem = {
  async handleStreamingWithAgents(res, namespace, message, history = [], schema = null, uploadedSchemas = [], allNamespaces = []) {
    const namespaceId = typeof namespace === 'object' && namespace !== null ? namespace.id : namespace;
    console.log('[LLM Agent] ===== handleStreamingWithAgents CALLED =====');
    console.log('[LLM Agent] Message:', message);
    console.log('[LLM Agent] Namespace ID:', namespaceId);
    console.log('[LLM Agent] All namespaces count:', allNamespaces?.length || 0);
    console.log('[LLM Agent] All namespaces:', JSON.stringify(allNamespaces, null, 2));
    res.writeHead(200, {
    ...
    console.log('[LLM Agent] Checking if message is resource query:', message);
    const isResource = isResourceQuery(message);
    console.log('[LLM Agent] isResourceQuery result:', isResource);
    console.log('[LLM Agent] allNamespaces received:', allNamespaces?.length || 0, 'namespaces');
    
    if (isResource) {
      console.log('[LLM Agent] Resource query detected for message:', message);
      console.log('[LLM Agent] allNamespaces received:', allNamespaces?.length || 0, 'namespaces');
      
      try {
        // Check if the query mentions a specific namespace name (like "shopify schemas")
        const mentionedNamespace = extractMentionedNamespace(message);
        console.log('[LLM Agent] Mentioned namespace:', mentionedNamespace);
        
        let allNamespaceData = [];
        
        if (mentionedNamespace) {
          // If a specific namespace is mentioned, fetch data for that namespace
          console.log('[LLM Agent] Fetching data for mentioned namespace:', mentionedNamespace);
          try {
            const namespaceData = await getRealNamespaceData(mentionedNamespace);
            // Get the actual namespace name from the data
            const actualNamespaceName = namespaceData.namespaceInfo?.['namespace-name'] || 'Unknown';
            allNamespaceData.push({ 
              namespace: { 'namespace-id': mentionedNamespace, 'namespace-name': actualNamespaceName }, 
              data: namespaceData 
            });
          } catch (err) {
            console.warn('[LLM Agent] Failed to fetch data for mentioned namespace', mentionedNamespace, err.message);
          }
        }
        
        // Also include namespaces from context (if not already included)
        if (allNamespaces && allNamespaces.length > 0) {
          console.log('[LLM Agent] Processing resource query with', allNamespaces.length, 'namespaces in context');
          for (const ns of allNamespaces) {
            const nsId = ns['namespace-id'] || ns.id;
            if (nsId && (!mentionedNamespace || nsId !== mentionedNamespace)) {
              try {
                const namespaceData = await getRealNamespaceData(nsId);
                allNamespaceData.push({
                  namespace: ns,
                  data: namespaceData
                });
        ...
        } else if (namespaceId && !mentionedNamespace) {
          console.log('[LLM Agent] Processing resource query with single namespace:', namespaceId);
        ...
        if (allNamespaceData.length > 0) {
          const resourceResponse = await handleResourceQueryWithMultipleNamespaces(message, allNamespaceData);
