...
function detectIntentWithGuidance(message) {
  ...
  const guidancePatterns = {
    ...
    schemas: /schema|json schema|data model|structure|validation/i,
    namespaces: /namespace|organize|library|manage/i,
    ...
    switch (feature) {
      ...
      case 'lambda':
        ...
        guidance.nextSteps = [
          ...
          "1. Go to the Lambda tab",
          "2. Provide a description of what you need",
          "3. I'll generate and deploy the function for you"
        ];
        guidance.uiActions = [
      ...
      case 'schemas':
        ...
        guidance.nextSteps = [
          ...
          "2. Create a new schema or import existing ones",
          "3. Use schemas to generate Lambda functions or validate data"
        ];
        guidance.uiActions = [
      ...
      case 'namespaces':
        ...
        guidance.nextSteps = [
          ...
          "1. Go to the Namespace Library tab",
          "2. View existing namespaces or create new ones",
          "3. Organize your work by project or service"
        ];
        guidance.uiActions = [
...
function isSchemaEditingIntent(message) {
  ...
  const editingPatterns = [
    ...
    /add\s+(?:mock\s+)?data/i,
    /generate\s+(?:mock\s+)?data/i,
    /create\s+(?:mock\s+)?data/i,
    /include\s+(?:mock\s+)?data/i,
    /add\s+(?:sample\s+)?data/i,
    /generate\s+(?:sample\s+)?data/i,
    /create\s+(?:sample\s+)?data/i,
    /include\s+(?:sample\s+)?data/i
  ];
  ...
  const editingKeywords = [
    ...
    'add mock data', 'generate mock data', 'create mock data', 'include mock data',
    'add sample data', 'generate sample data', 'create sample data', 'include sample data',
    'mock data', 'sample data', 'test data', 'example data'
  ];
...
export const agentSystem = {
  async handleStreamingWithAgents(res, namespace, message, history = [], schema = null) {
    ...
    if (intentWithGuidance.shouldProvideGuidance) {
      ...
      res.write(`data: ${JSON.stringify(guidanceResponse)}\n\n`);
      
      // Send a helpful chat message as well
      const chatMessage = {
        type: 'chat',
        content: `I can help you with ${intentWithGuidance.feature}! ${guidance.suggestions.join(' ')}`,
        route: 'guidance'
      };
          ...
          for await (const chunk of stream) {
            if (chunk.type === 'content_block_delta') {
              ...
              if (content.includes('{') && !hasStartedJson) {
                hasStartedJson = true;
                // Send a message indicating schema editing is starting
                res.write(`data: ${JSON.stringify({ content: "\n\n🔄 Editing schema...", type: 'chat', route: 'chat' })}\n\n`);
              }
...
1. SCHEMA WORKSPACE:
   - You're currently in the Schema tab where users can create and edit JSON schemas
   - Schemas can be used to generate Lambda functions, validate data, and organize information
   - Users can edit schemas through natural language instructions
...
8. Guide users to other workspace features when relevant (e.g., "You can generate a Lambda function from this schema using the Lambda tab")
9. Suggest next steps like generating Lambda functions from schemas or organizing in namespaces
...
Answer the user's questions conversationally, provide guidance on workspace features, and help them navigate and use the platform effectively.`;
    ...
    try {
      ...
      res.write(`data: ${JSON.stringify({ actions: [{ type: 'error', status: 'failed', message: error.message }], type: 'actions', route: 'schema' })}\n\n`);
    } finally {
      res.end();
    }
  }
