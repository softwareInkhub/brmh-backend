...
function detectIntentWithGuidance(message) {
  ...
  const guidancePatterns = {
    ...
    lambda: /lambda|function|serverless|aws lambda|generate function|deploy/i,
...
- If schemas are provided, use them appropriately in your function logic`;
  ...
  try {
    if (res) {
      ...
      // Send the structured Lambda code response that the frontend expects
      if (fullCode && fullCode.length > 0) {
        // Prefer explicit user-provided functionName for display
        const displayFunctionName = functionName || (selectedSchema ? `${selectedSchema.schemaName || selectedSchema.name}Handler` : 'GeneratedHandler');
        const lambdaResponse = {
          type: 'lambda_code',
          schemaName: displayFunctionName,
          schema: selectedSchema,
          code: fullCode,
          route: 'lambda'
        };
        
        console.log('[LLM Agent] Sending structured Lambda response:', {
          type: lambdaResponse.type,
          schemaName: lambdaResponse.schemaName,
          codeLength: lambdaResponse.code.length,
          route: lambdaResponse.route
        });
        ...
        const chatMessage = {
          type: 'chat',
          content: `✅ Generated Lambda function: **${lambdaResponse.schemaName}**\n\nCheck the Lambda tab to see the generated code!`,
          route: 'chat'
        };
...
export const agentSystem = {
  async handleStreamingWithAgents(res, namespace, message, history = [], schema = null) {
    ...
    // Check if user is responding with schema names for Lambda generation (conversational flow)
    if (namespaceId && !intent.shouldGenerateLambda && !intent.shouldGenerateSchema) {
      try {
        const namespaceData = await getRealNamespaceData(namespaceId);
        const availableSchemas = namespaceData.schemas || [];
        const messageLower = message.toLowerCase();
        
        // Look for schema names in the message
        let selectedSchemas = [];
        for (const schema of availableSchemas) {
          const schemaName = (schema.schemaName || schema.name || '').toLowerCase();
          if (schemaName && messageLower.includes(schemaName)) {
            selectedSchemas.push(schema);
          }
        }
        
        // Check for "all schemas" or similar requests
        const allSchemasKeywords = ['all schemas', 'all of them', 'every schema', 'combine all', 'use all'];
        const hasAllSchemasIntent = allSchemasKeywords.some(keyword => messageLower.includes(keyword));
        
        // If user mentioned schemas and we have schemas available, treat this as a Lambda generation request
        if (selectedSchemas.length > 0 || (hasAllSchemasIntent && availableSchemas.length > 0)) {
          console.log('[LLM Agent] Schema selection detected in conversational flow:', selectedSchemas.map(s => s.schemaName || s.name));
          
          // Use all schemas if "all schemas" was requested
          const schemasToUse = hasAllSchemasIntent ? availableSchemas : selectedSchemas;
          
          const lambdaCodegenParams = {
            message: `Generate a Lambda function using the ${schemasToUse.map(s => s.schemaName || s.name).join(', ')} schema(s)`,
            selectedSchema: schemasToUse.length === 1 ? schemasToUse[0] : null,
            // Respect user-provided functionName in conversational flow if present in history/context
            functionName: functionName || (schemasToUse.length === 1 ? `${schemasToUse[0].schemaName || schemasToUse[0].name}Handler` : 'MultiSchemaHandler'),
            runtime: 'nodejs18.x',
          ...
          if (result.generatedCode) {
            const lambdaResponse = {
              type: 'lambda_code',
              schemaName: schemasToUse.length === 1 ? (schemasToUse[0].schemaName || schemasToUse[0].name) : 'MultiSchema',
              schema: schemasToUse.length === 1 ? schemasToUse[0] : { name: 'MultiSchema', schemas: schemasToUse },
            ...
            const schemaNames = schemasToUse.map(s => s.schemaName || s.name).join(', ');
            const chatMessage = {
              type: 'chat',
              content: `✅ Perfect! Generated Lambda function using schema(s): **${schemaNames}**\n\nCheck the Lambda tab to see the generated code!`,
              route: 'chat'
            };
        ...
        console.warn('[LLM Agent] Failed to handle conversational schema selection:', err.message);
      }
    }
    ...
    if (intent.shouldGenerateLambda && namespaceId) {
      console.log('[LLM Agent] Lambda generation request detected');
      ...
      try {
        ...
        if (hasMultipleSchemaIntent && availableSchemas.length > 1) {
          console.log('[LLM Agent] Multiple schema Lambda generation requested');
            ...
            message: message,
            selectedSchema: null,
              functionName: functionName || 'MultiSchemaHandler',
            runtime: 'nodejs18.x',
            handler: 'index.handler',
            memory: 256,
            timeout: 30,
          ...
          if (result.generatedCode) {
            const lambdaResponse = {
              type: 'lambda_code',
              schemaName: 'MultiSchema',
              schema: { name: 'MultiSchema', schemas: availableSchemas },
              code: result.generatedCode,
              route: 'lambda'
            };
            ...
            const chatMessage = {
              type: 'chat',
              content: `✅ Generated Lambda function using ${availableSchemas.length} schemas: **${availableSchemas.map(s => s.schemaName || s.name).join(', ')}**\n\nCheck the Lambda tab to see the generated code!`,
              route: 'chat'
            };
        ...
        // If specific schemas were mentioned, generate Lambda with those schemas
        if (selectedSchemas.length > 0) {
          console.log('[LLM Agent] Found schemas for Lambda generation:', selectedSchemas.map(s => s.schemaName || s.name));
          
          const lambdaCodegenParams = {
            message: message,
            selectedSchema: selectedSchemas.length === 1 ? selectedSchemas[0] : null,
            functionName: functionName || (selectedSchemas.length === 1 ? `${selectedSchemas[0].schemaName || selectedSchemas[0].name}Handler` : 'MultiSchemaHandler'),
            runtime: 'nodejs18.x',
            handler: 'index.handler',
            memory: 256,
            timeout: 30,
            environment: null,
            namespace: namespaceId,
            res: null, // Don't stream directly, capture the result
            uploadedSchemas: selectedSchemas // Use the selected schemas
          };
          
          // Call the existing Lambda codegen handler and capture the result
          const result = await handleLambdaCodegen(lambdaCodegenParams);
          
          if (result.generatedCode) {
            // Send Lambda code generation response in the format the frontend expects
            const lambdaResponse = {
              type: 'lambda_code',
              schemaName: selectedSchemas.length === 1 ? (selectedSchemas[0].schemaName || selectedSchemas[0].name) : 'MultiSchema',
              schema: selectedSchemas.length === 1 ? selectedSchemas[0] : { name: 'MultiSchema', schemas: selectedSchemas },
              code: result.generatedCode,
              route: 'lambda'
            };
            ...
            // Also send a chat message
            const schemaNames = selectedSchemas.map(s => s.schemaName || s.name).join(', ');
            const chatMessage = {
              type: 'chat',
              content: `✅ Generated Lambda function using schema(s): **${schemaNames}**\n\nCheck the Lambda tab to see the generated code!`,
              route: 'chat'
            };
          ...
          // Show available schemas for selection - conversational approach
          const schemaList = availableSchemas.map((schema, index) => 
            `• **${schema.schemaName || schema.name}** (${schema.schemaType || schema.type || 'JSON'})`
          ).join('\n');
          
          const chatMessage = {
            type: 'chat',
            content: `I'd be happy to generate a Lambda function for you! 🚀\n\n**Available schemas in your namespace:**\n${schemaList}\n\n**Just tell me which schema(s) you'd like to use:**\n• Say the schema name (e.g., "${availableSchemas[0].schemaName || availableSchemas[0].name}")\n• Or mention multiple schemas (e.g., "${availableSchemas[0].schemaName || availableSchemas[0].name} and ${availableSchemas.length > 1 ? availableSchemas[1].schemaName || availableSchemas[1].name : 'another schema'}")\n• Or ask for "all schemas" to combine them\n\nI'll automatically generate the Lambda function and show it in the Lambda tab!`,
            route: 'chat'
          };
          ...
          const chatMessage = {
            type: 'chat',
            content: `I'd love to help you generate a Lambda function! However, I don't see any schemas in your current namespace.\n\n**To get started:**\n• Create a schema first using the Schema tab\n• Or upload a schema file\n• Then I can generate a Lambda function for you!`,
            route: 'chat'
          };
