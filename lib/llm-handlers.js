...
Do not include any explanations, markdown, or code fences - just the JSON object.`;
  ...
  // New handler for generating Lambda code from description
  generateLambdaCode: async (c, req, res) => {
    try {
      const { functionName, description, runtime, memorySize, timeout } = c.request.requestBody;
      if (!functionName || !description) {
        return { statusCode: 400, body: { error: 'Function name and description are required' } };
      }

      const systemPrompt = `You are an expert AWS Lambda function generator. Based on the user's description, generate a complete Lambda function code that:

1. Handles the described functionality
2. Includes proper error handling
3. Returns appropriate HTTP responses
4. Follows AWS Lambda best practices
5. Includes CORS headers for web access
6. Logs important events
7. Validates input when necessary

Return ONLY the JavaScript code for the Lambda function. Do not include any explanations, markdown, or code fences - just the pure JavaScript code.`;

      const fullPrompt = `${systemPrompt}\n\nFunction Name: ${functionName}\nDescription: ${description}\nRuntime: ${runtime || 'nodejs18.x'}\nMemory: ${memorySize || 128}MB\nTimeout: ${timeout || 30}s`;
  ...
  // New handler for complete namespace automation
  automateNamespaceCreation: async (c, req, res) => {
    try {
      const { namespaceName, userPrompt } = c.request.requestBody;
      if (!namespaceName || !userPrompt) {
        return { statusCode: 400, body: { error: 'Namespace name and user prompt are required' } };
      }

      const systemPrompt = `You are an expert backend system architect. Based on the user's description, create a complete namespace with schemas, methods, and Lambda functions.

Generate a comprehensive backend system that includes:

1. **Schemas**: All necessary data models and validation schemas
2. **Methods**: RESTful API endpoints with proper HTTP methods
3. **Lambda Functions**: Serverless functions for each method
4. **Documentation**: Clear descriptions for each component
...
Return ONLY a valid JSON object with this structure:
{
  ...
  "summary": "Brief summary of the complete system"
}

Do not include any explanations, markdown, or code fences - just the JSON object.`;

      const fullPrompt = `${systemPrompt}\n\nNamespace Name: ${namespaceName}\nApplication Description: ${userPrompt}`;
