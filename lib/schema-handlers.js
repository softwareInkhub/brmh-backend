...
const getSchemaByTableName = async (tableName) => {
  // Find the schema record by tableName in brmh-schema-table-data
...
const checkAllTableStatuses = async () => {
  ...
  for (const meta of metaItems) {
    ...
    if (!isActive && meta.status !== 'TABLE_DELETED') {
      // Update status in brmh-schema-table-data
      await docClient.send(new UpdateCommand({
        TableName: 'brmh-schema-table-data',
        Key: { id: meta.id },
        UpdateExpression: 'SET #status = :status',
        ExpressionAttributeNames: { '#status': 'status' },
        ExpressionAttributeValues: { ':status': 'TABLE_DELETED' }
      }));
      updated++;
