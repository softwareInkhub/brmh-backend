...
export class LambdaDeploymentManager {
  ...
  async storeDeploymentMetadata(deploymentData) {
    try {
      ...
      const item = {
        ...
        namespaceId: { S: deploymentData.namespaceId || 'general' }
      };
  ...
  async listDeployments(functionName = null, namespaceIds = null) {
    try {
      console.log(`[DynamoDB] Listing deployments`, { functionName, namespaceIds });
      
      // Use Scan instead of Query to filter by namespaceId
      const scanParams = {
        TableName: DYNAMODB_TABLE_NAME
      };
      ...
      if (functionName) {
        items = items.filter(item => item.functionName?.S === functionName);
      }
      
      // Filter by namespaceIds if provided
      if (namespaceIds) {
        const parseIds = (val) => {
          if (!val) return [];
        ...
        const allowedIds = new Set(parseIds(namespaceIds));
        if (allowedIds.size > 0) {
          items = items.filter(item => allowedIds.has(item.namespaceId?.S));
        }
      }
      ...
      return items.map(item => ({
        ...
        status: item.status?.S || 'deployed',
        deployedAt: item.deployedAt?.S || '',
        apiGatewayUrl: item.apiGatewayUrl?.S || '',
        namespaceId: item.namespaceId?.S || null
      }));
    } catch (error) {
  ...
  async deployLambdaFunction(functionName, code, runtime = 'nodejs18.x', handler = 'index.handler', memorySize = 128, timeout = 30, dependencies = {}, environment = '', createApiGateway = true, namespaceId = null) {
    try {
      ...
      if (functionExists) {
        ...
        if (createApiGateway) {
          try {
            ...
            console.log(`[Lambda Deployment] ✅ API Gateway created successfully: ${apiGatewayResult.apiGatewayUrl}`);
          } catch (apiError) {
            console.error(`[Lambda Deployment] ❌ API Gateway creation failed: ${apiError.message}`);
            console.error(`[Lambda Deployment] API Gateway error details:`, apiError);
            // Continue with deployment even if API Gateway fails, but log the error clearly
            deploymentData.apiGatewayError = apiError.message;
          }
        }
        ...
        return {
          ...
          apiGatewayError: deploymentData.apiGatewayError || null
        };
      } else {
        // Create new function
  ...
  async createApiGateway(functionName, functionArn, runtime, handler, deploymentId = null) {
    try {
      ...
      try {
        const stsResponse = await stsClient.send(new GetCallerIdentityCommand({}));
        console.log(`[API Gateway] AWS credentials validated. Account: ${stsResponse.Account}, User: ${stsResponse.Arn}`);
      } catch (credError) {
        console.error(`[API Gateway] AWS credentials validation failed:`, credError);
        throw new Error(`AWS credentials invalid: ${credError.message}`);
      }
      ...
      try {
        ...
        console.warn('[REST API] Error details:', {
          name: restErr.name,
          code: restErr.code,
      ...
      if (error.name === 'AccessDeniedException') {
        errorMessage = `Access denied. Check your AWS permissions for API Gateway and Lambda. Original error: ${error.message}`;
      } else if (error.name === 'InvalidParameterValueException') {
        errorMessage = `Invalid parameter. Check function ARN and region. Original error: ${error.message}`;
      } else if (error.name === 'TooManyRequestsException') {
        errorMessage = `Rate limit exceeded. Wait a few minutes and try again. Original error: ${error.message}`;
      }
  ...
  async deleteLambdaFunction(functionName) {
    try {
      ...
      // Import DeleteFunctionCommand
      const { DeleteFunctionCommand } = await import('@aws-sdk/client-lambda');
      
      // Delete from AWS Lambda
      try {
        await lambdaClient.send(new DeleteFunctionCommand({
          FunctionName: functionName
        }));
        console.log(`[Lambda Deletion] Successfully deleted from AWS: ${functionName}`);
      } catch (awsError) {
        console.warn(`[Lambda Deletion] AWS deletion failed (may not exist): ${awsError.message}`);
      }
      
      // Delete from DynamoDB - scan for all deployments with this function name
      try {
        const scanResponse = await dynamoDBClient.send(new ScanCommand({
          TableName: DYNAMODB_TABLE_NAME
        }));
        
        const deploymentsToDelete = (scanResponse.Items || []).filter(item => 
        ...
        for (const deployment of deploymentsToDelete) {
          ...
          console.log(`[Lambda Deletion] Deleted deployment metadata: ${deployment.deploymentId.S}`);
        }
        
        console.log(`[Lambda Deletion] Deleted ${deploymentsToDelete.length} deployment record(s) from DynamoDB`);
      } catch (dbError) {
        console.warn(`[Lambda Deletion] DynamoDB deletion failed: ${dbError.message}`);
      }
      
      return {
        success: true,
        message: `Successfully deleted Lambda function: ${functionName}`,
        functionName: functionName
      };
    } catch (error) {
