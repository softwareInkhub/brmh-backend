...
import { ApiGatewayV2Client, CreateApiCommand, CreateIntegrationCommand, CreateRouteCommand, CreateStageCommand, GetApiCommand, GetApisCommand, GetIntegrationsCommand, GetRoutesCommand, GetStagesCommand } from '@aws-sdk/client-apigatewayv2';
import fs from 'fs';
...
// Function to sanitize Lambda function names to comply with AWS naming requirements
function sanitizeFunctionName(name) {
  if (!name) return 'LambdaFunction';
  
  // AWS Lambda function name constraints:
  ...
  if (!sanitized || sanitized.length === 0) {
    sanitized = 'lambda-function';
  }
  
  // Truncate if too long (AWS limit is 64 characters)
  if (sanitized.length > 64) {
    sanitized = sanitized.substring(0, 64).replace(/-+$/, '');
  }
  
  // Ensure it doesn't start or end with a hyphen after truncation
...
export class LambdaDeploymentManager {
  ...
  async deployLambdaFunction(functionName, code, runtime = 'nodejs18.x', handler = 'index.handler', memorySize = 128, timeout = 30, dependencies = {}, environment = '', createApiGateway = true) {
    let sanitizedFunctionName;
    let originalFunctionName = functionName;
    try {
      // Sanitize function name to comply with AWS naming requirements
      sanitizedFunctionName = sanitizeFunctionName(functionName);
      
      if (originalFunctionName !== sanitizedFunctionName) {
        console.log(`[Lambda Deployment] Function name sanitized: "${originalFunctionName}" -> "${sanitizedFunctionName}"`);
      }
      
      console.log(`[Lambda Deployment] Deploying function: ${sanitizedFunctionName}`);
      ...
      // Generate deployment ID using sanitized name
      const deploymentId = `${sanitizedFunctionName}-${Date.now()}`;
      ...
      const zipPath = await this.createDeploymentPackage(sanitizedFunctionName, code, dependencies);
      
      // Upload to S3
      console.log(`[Lambda Deployment] Uploading deployment package to S3...`);
      const s3Result = await this.uploadToS3(sanitizedFunctionName, zipPath, deploymentId);
      ...
      if (!roleArn) {
        ...
        const roleName = `${sanitizedFunctionName}-execution-role`;
      ...
      try {
        await lambdaClient.send(new GetFunctionCommand({ FunctionName: sanitizedFunctionName }));
        functionExists = true;
        console.log(`[Lambda Deployment] Function ${sanitizedFunctionName} already exists, updating...`);
      } catch (error) {
        if (error.name !== 'ResourceNotFoundException') {
          throw error;
        }
        console.log(`[Lambda Deployment] Function ${sanitizedFunctionName} does not exist, creating...`);
      }
      ...
      if (functionExists) {
        ...
        const deploymentData = {
          ...
          description: `Generated Lambda function: ${sanitizedFunctionName}`,
          timeout: timeout,
        ...
        if (createApiGateway) {
          try {
            console.log(`[Lambda Deployment] Creating API Gateway for function: ${sanitizedFunctionName}`);
            apiGatewayResult = await this.createApiGateway(sanitizedFunctionName, updateResponse.FunctionArn, runtime, handler, deploymentId);
        ...
        if (createApiGateway) {
          try {
            console.log(`[Lambda Deployment] Creating API Gateway for function: ${sanitizedFunctionName}`);
            apiGatewayResult = await this.createApiGateway(sanitizedFunctionName, createResponse.FunctionArn, runtime, handler, deploymentId);
      ...
      // Clean up temp files
      await this.cleanupTempFiles(sanitizedFunctionName);
    }
  }

  async invokeLambdaFunction(functionName, payload = {}) {
    try {
      // Ensure we use sanitized name for invocation as well
      const sanitizedFunctionName = sanitizeFunctionName(functionName);
      console.log(`[Lambda Deployment] Invoking function: ${sanitizedFunctionName}`);
  ...
  async createApiGateway(functionName, functionArn, runtime, handler, deploymentId = null) {
    try {
      // Ensure we use sanitized name for API Gateway creation as well
      const sanitizedFunctionName = sanitizeFunctionName(functionName);
      // Derive region/account from functionArn as a resilient fallback
      const arnParts = (functionArn || '').split(':');
      const derivedRegion = arnParts[3] || process.env.AWS_REGION || 'us-east-1';
      const derivedAccountId = arnParts[4] || process.env.AWS_ACCOUNT_ID || '123456789012';
      console.log(`[API Gateway] Creating API Gateway for function: ${sanitizedFunctionName}`);
      ...
      console.log(`[API Gateway] AWS Region: ${derivedRegion}`);
      console.log(`[API Gateway] AWS Account ID: ${derivedAccountId}`);
      ...
      if (!process.env.AWS_ACCOUNT_ID) {
        console.warn(`[API Gateway] AWS_ACCOUNT_ID not set, using derived: ${derivedAccountId}`);
      }
      
      // Check if API already exists for this function
      let apiId = null;
      let apiName = `lambda-api-${sanitizedFunctionName}`;
      ...
      try {
        ...
        if (error.name === 'ConflictException' || error.name === 'ResourceConflictException') {
          console.warn(`[API Gateway] Integration already exists, attempting to reuse.`);
          try {
            const existing = await apiGatewayClient.send(new GetIntegrationsCommand({ ApiId: apiId }));
            const found = existing.Items?.find(i => i.IntegrationUri === functionArn) || existing.Items?.[0];
            if (!found) throw error;
            integrationResponse = { IntegrationId: found.IntegrationId };
            console.log(`[API Gateway] Reusing integration: ${found.IntegrationId}`);
          } catch (listErr) {
            console.error(`[API Gateway] Failed to list/reuse integrations:`, listErr);
            throw new Error(`Failed to create or reuse integration: ${error.message}`);
          }
        } else {
      ...
      try {
        ...
        routeResponse = await apiGatewayClient.send(new CreateRouteCommand({
          ApiId: apiId,
          RouteKey: `POST /${sanitizedFunctionName}`,
          Target: `integrations/${integrationResponse.IntegrationId}`
        }));
        console.log(`[API Gateway] Created route: ${routeResponse.RouteId}`);
      } catch (error) {
        if (error.name === 'ConflictException' || error.name === 'ResourceConflictException') {
          console.warn(`[API Gateway] Route already exists, attempting to reuse.`);
          try {
            const routes = await apiGatewayClient.send(new GetRoutesCommand({ ApiId: apiId }));
            const found = routes.Items?.find(r => r.RouteKey === `POST /${sanitizedFunctionName}`) || routes.Items?.[0];
            if (!found) throw error;
            routeResponse = { RouteId: found.RouteId };
            console.log(`[API Gateway] Reusing route: ${found.RouteId}`);
          } catch (listErr) {
            console.error(`[API Gateway] Failed to list/reuse routes:`, listErr);
            throw new Error(`Failed to create or reuse route: ${error.message}`);
          }
        } else {
      ...
      try {
        ...
        if (error.name === 'ConflictException' || error.name === 'ResourceConflictException') {
          console.warn(`[API Gateway] Stage already exists, attempting to reuse.`);
          try {
            const stages = await apiGatewayClient.send(new GetStagesCommand({ ApiId: apiId }));
            const found = stages.Items?.find(s => s.StageName === 'prod') || stages.Items?.[0];
            if (!found) throw error;
            stageResponse = { StageName: found.StageName };
            console.log(`[API Gateway] Reusing stage: ${found.StageName}`);
          } catch (listErr) {
            console.error(`[API Gateway] Failed to list/reuse stages:`, listErr);
            throw new Error(`Failed to create or reuse stage: ${error.message}`);
          }
        } else {
    ...
    try {
      ...
      await lambdaClient.send(new AddPermissionCommand({
        FunctionName: sanitizedFunctionName,
          ...
          SourceArn: `arn:aws:execute-api:${derivedRegion}:${derivedAccountId}:${apiId}/*/*/${sanitizedFunctionName}`
        }));
