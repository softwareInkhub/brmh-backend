...
export async function generateStepFunctionsDefinition(workflow) {
  ...
  for (const step of steps) {
    ...
    if (type === 'api') {
      ...
      // Build HTTP task state using /execute endpoint format
      // AWS Step Functions HTTP invoke requires Method field
      const httpTaskParams = {
        ApiEndpoint: apiParams.url,
        Method: (apiParams.method || 'POST').toUpperCase(), // Required by AWS Step Functions (must be uppercase)
        RequestBody: apiParams.body // Already formatted for /execute endpoint
      };
      
      // Add Headers if present (optional but recommended)
      if (apiParams.headers && Object.keys(apiParams.headers).length > 0) {
        httpTaskParams.Headers = apiParams.headers;
      }
      
      // ConnectionArn is optional - only needed for OAuth/API key auth via AWS Secrets Manager
      // For basic auth (Bearer tokens in headers), we don't need InvocationConfig or Authentication
      // IMPORTANT: Only add InvocationConfig if connectionArn is a valid ARN
      const connectionArn = step.connectionArn;
      const hasValidConnectionArn = connectionArn && 
                                    typeof connectionArn === 'string' && 
                                    connectionArn.trim() !== '' && 
                                    connectionArn.trim().startsWith('arn:aws:');
      
      if (hasValidConnectionArn) {
        httpTaskParams.InvocationConfig = {
          ConnectionArn: connectionArn.trim()
        };
        console.log(`[Workflow] Added InvocationConfig with ConnectionArn for step ${id}:`, connectionArn.trim());
      }
      ...
      if (onSuccess && onFailure) {
        // Both success and failure handlers - use Choice state
        const choiceStateName = `${stateName}Choice`;
        nextState = choiceStateName;
        // Store the choice state name mapping
        stepIdToStateName[`${id}Choice`] = choiceStateName;
      } else if (onSuccess) {
        // Only success handler - use Choice state
        const choiceStateName = `${stateName}Choice`;
        nextState = choiceStateName;
        stepIdToStateName[`${id}Choice`] = choiceStateName;
      } else if (next) {
        // Direct next step - map to state name
      ...
      // Build HTTP task state - using Arguments format to match Step Functions Studio format
      const httpTaskState = {
        Type: 'Task',
        Resource: 'arn:aws:states:::http:invoke',
        Arguments: httpTaskParams, // Using Arguments instead of Parameters to match Step Functions Studio
        Retry: [retryConfig],
      ...
      // If we have both onSuccess and onFailure, add a Choice state
      if (onSuccess && onFailure) {
        const choiceStateName = stepIdToStateName[`${id}Choice`] || `${stepIdToStateName[id]}Choice`;
        ...
        // Add Choice state to check success (2xx status codes)
        const choiceStateName = stepIdToStateName[`${id}Choice`] || `${stepIdToStateName[id]}Choice`;
        const successStateName = stepIdToStateName[onSuccess] || onSuccess;
  ...
  return {
    ...
    StartAt: startAt,
    States: states,
    QueryLanguage: 'JSONata' // Add JSONata query language support
  };
}
