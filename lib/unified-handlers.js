    ...
    (async () => {
      try {
        ...
        async function saveItemsToDynamoDB(items, pageData, grandTotalObj) {
          ...
          // Helper function to extract nested items from arrays
          const extractNestedItems = (obj, parentKey = '') => {
            const extractedItems = [];
            
            Object.entries(obj).forEach(([key, value]) => {
              if (Array.isArray(value)) {
                // If it's an array, extract each item
                value.forEach((arrayItem, index) => {
                  if (typeof arrayItem === 'object' && arrayItem !== null) {
                    // Create a new item with the array item's data
                    const nestedItem = { ...arrayItem };
                    
                    // Add parent context if needed
                    if (parentKey) {
                      nestedItem._parent_key = parentKey;
                      nestedItem._parent_index = index;
                    }
                    
                    extractedItems.push(nestedItem);
                  }
                });
              } else if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
                // If it's an object, recursively extract
                const nestedItems = extractNestedItems(value, key);
                extractedItems.push(...nestedItems);
              } else {
          ...
          for (let i = 0; i < items.length; i++) {
            ...
            // Extract nested items from arrays
            const extractedItems = extractNestedItems(item);
            
            // If we extracted nested items, process them instead of the original item
            const itemsToProcess = extractedItems.length > 0 ? extractedItems : [item];
            ...
            for (let j = 0; j < itemsToProcess.length; j++) {
              ...
              if (!cleanItem.id) {
                ...
                const idField = Object.keys(cleanItem).find(key => 
              ...
              console.log(`Batch ${i + 1}/${items.length}, Item ${j + 1}/${itemsToProcess.length}: Saving item to DynamoDB`);
              try {
                ...
                console.error(`Error saving item ${i + 1}-${j + 1}:`, error.message);
              }
...
const getSchemasForSelection = async (c, req, res) => {
  try {
    ...
    const { search = '', limit = 50, schemaType = '' } = c.request.query || {};
    console.log('Query params:', { search, limit, schemaType });
    ...
    if (search) {
      filteredSchemas = allSchemas.filter(schema => 
        ...
        schema.methodName?.toLowerCase().includes(search.toLowerCase())
      );
    }
    console.log('Filtered schemas count:', filteredSchemas.length);
    
    // Optional schemaType filter
    if (schemaType) {
      filteredSchemas = filteredSchemas.filter(s => (s.schemaType || '').toLowerCase() === String(schemaType).toLowerCase());
    }
    // Limit results
    ...
    const schemasForSelection = limitedSchemas.map(schema => ({
      ...
      schemaType: schema.schemaType || null
    }));
