...
export const handlers = {
  ...
  // Enhanced AI Agent with streaming and file operations
  aiAgent: async (c, req, res) => {
    try {
      ...
      if (stream) {
        await streamingAIAgent.streamResponse(res, namespaceId, message, history);
        return null; // Response is handled by streaming
      }
      // Non-streaming response for compatibility
      const intent = streamingAIAgent.detectIntent(message);
      // Handle file operations
      if (intent === 'create_file' || intent === 'edit_file') {
        const fileResult = await handleFileOperationFromMessage(message, namespaceId);
        return {
          statusCode: 200,
          body: {
            ...
            type: 'file',
            fileOperation: fileResult.operation
          }
        };
      }
      // Handle project setup
      if (intent === 'project_setup') {
        const projectResult = await handleProjectSetup(message, namespaceId);
        return {
          statusCode: 200,
          body: {
            content: projectResult.content,
            output: projectResult.output,
            type: 'project',
      ...
      const response = await generateDefaultResponse(intent, message, namespaceId, history);
      return {
        statusCode: 200,
        body: response
      };
  ...
  fileOperations: async (c, req, res) => {
    try {
      const { operation, namespaceId, filePath, content } = c.request.requestBody || {};
      
      const result = await streamingAIAgent.handleFileOperation(namespaceId, operation, filePath, content);
  ...
  // Get file tree for namespace
  getFileTree: async (c, req, res) => {
    try {
      const { namespaceId } = c.request.params;
      const result = await fileOperations.getFileTree(namespaceId);
