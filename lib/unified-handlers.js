        ...
        while (hasMorePages && (maxIterations === null || pageCount <= maxIterations)) {
          ...
          if (detectedPaginationType === 'link') {
            ...
            if (!nextUrl) {
              ...
              // Always preserve the original status parameter from the request
              // This ensures we get the same order status across all pages
              if (queryParams.status) {
                nextUrlObj.searchParams.set('status', queryParams.status);
                console.log(`Setting status parameter to: ${queryParams.status} for page ${pageCount + 1}`);
              } else {
                // If no status was provided, default to 'all' to get all orders
                nextUrlObj.searchParams.set('status', 'all');
                console.log(`No status provided, defaulting to 'all' for page ${pageCount + 1}`);
              }
