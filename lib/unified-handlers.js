...
Be concise but thorough. If the user asks for specific functionality, provide the actual code or configuration needed.`;
    ...
    // Determine response type based on user intent first, then content
    ...
    if (userMessageLower.includes('schema') || userMessageLower.includes('model') || userMessageLower.includes('database') || userMessageLower.includes('json schema')) {
    ...
    } else if (userMessageLower.includes('test') || userMessageLower.includes('debug')) {
      responseType = 'test';
      metadata = {
        type: 'test',
        data: {
    ...
    } else if (userMessageLower.includes('auth') || userMessageLower.includes('login')) {
      responseType = 'auth';
      metadata = {
        type: 'auth',
        data: {
      ...
      // Fallback: check AI response content
      if (aiResponseLower.includes('api') || aiResponseLower.includes('endpoint')) {
        responseType = 'api';
        metadata = {
          type: 'api',
          data: {
      ...
      } else if (aiResponseLower.includes('schema') || aiResponseLower.includes('model')) {
        responseType = 'schema';
        metadata = {
          type: 'schema',
          data: {
      ...
      } else if (aiResponseLower.includes('test') || aiResponseLower.includes('debug')) {
        responseType = 'test';
        metadata = {
          type: 'test',
          data: {
      ...
      } else if (aiResponseLower.includes('auth') || aiResponseLower.includes('login')) {
        responseType = 'auth';
        metadata = {
          type: 'auth',
          data: {
