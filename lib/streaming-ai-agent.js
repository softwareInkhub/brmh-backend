...
export class StreamingAIAgent {
  ...
  async streamResponse(res, namespace, message, history = []) {
    ...
    try {
      ...
      // Use orchestrator for intelligent task handling
      const orchestrationResult = await orchestrateAIAgentTask({
        message,
        namespace: namespaceContext,
        history,
        session: this.memory.get(namespaceId) || {}
      });
      
      // Stream the orchestrated response
      await this.streamOrchestratedResponse(res, orchestrationResult);
...
Intent detected: ${intent}`;
  ...
  async streamOrchestratedResponse(res, orchestrationResult) {
    const { response, actions } = orchestrationResult;
    ...
    // Send actions data
    res.write(`data: ${JSON.stringify({ actions, type: 'actions' })}\n\n`);
  }
